  ...
  const filteredPosts = useMemo(() => {
    return boardData
      ...
      .filter(post => {
        ...
        // 로그인 상태에서 필터링
        if (isLoggedIn) {
          // 설정된 privateType이 'public'일 경우 게시글의 privateType이 'private'이면서 작성자가 로그인한 사용자일 때만 보이게 함
          return post.privateType === privateType &&
                 (post.privateType === 'private' && post.username === loggedInUsername);
        }

        // 로그아웃 상태에서는 private 게시물에 대한 특별 처리가 없으며, 공개 게시물만 필터링
       // return post.privateType === 'public';
      });
