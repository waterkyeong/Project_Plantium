...
const EditPostPage = () => {
    ...
    const fetchPost = async () => {
      try {
        ...
        if (data.fimges && data.fimges.length > 0) {
          ...
          console.log('파일데이터 이름', data.fimges);
  ...
  const handleRemoveFile = (file) => {
    ...
    setFiles(files => files.filter(f => f !== file)); // UI에서 파일 제거
    
    if (file.existing) {
      setRemovedFiles(files => [...files, file.imgname]); // 삭제된 기존 파일 ID 저장
    }
  ...
  const handleSubmit = async (e) => {
    ...
    console.log("삭제할 파일",removedFiles);
